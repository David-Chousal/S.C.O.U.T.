...
- **Delivered** — [PR #46](https://github.com/David-Chousal/S.C.O.U.T./pull/46): CR 4/8 modem config, blind repetition in the new `scout_link` lib (3 copies NORMAL / 1 CONSERVE / 0 CRITICAL, widening gaps, watchdog-headroom guard), and shore-side dedupe on `(buoy_id, record_seq)` so the copies collapse to one row. SF left at 7 pending **B6**/[SCO-19](https://linear.app/scout1/issue/SCO-19). Keep this issue open until the Phase 4 range test measures the actual delivery gain (**B1**/[SCO-14](https://linear.app/scout1/issue/SCO-14)).
