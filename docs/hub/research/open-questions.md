...
| Mooring scope and line unit weight for the calm-water catenary load case | [Buoy Structural Load Framework §8.2](../../engineering/buoy-structural-load-framework.md#82-lc-a--slack-mooring-calm-water-baseline-catenary-the-physically-correct-resting-state) needs the actual scope length `S` and line submerged unit weight `w_m` to compute the real calm-water shackle tension; the ChatGPT-drafted FEA framework had assumed a taut mooring, which contradicts ADR-0004's slack/swing-radius design | John (GENG) | Not started — depends on [SCO-17](https://linear.app/scout1/issue/SCO-17) mooring hardware CAD |
| Final Cd/Cm/Ca coefficients for the structural load FEA cases | Needs Reynolds number (current) and Keulegan–Carpenter number (wave) computed from real geometry and chosen design environmental values, then pulled from [`dnv-rp-c205`](sources.md#structural--hydrodynamic-loads-fea) — not assumed from a generic cylinder-drag table | John (GENG) | Not started — [Buoy Structural Load Framework §3.1](../../engineering/buoy-structural-load-framework.md#31-correction--how-to-actually-select-c_d-and-c_m-c_a-in-5) |
