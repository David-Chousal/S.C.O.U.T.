...
**Why it matters to SCOUT.** This is the exact algorithm SCOUT's telemetry `bleaching.py` implements from
in-situ temperature. The value SCOUT adds is *where the satellite is weakest*: the 5 km product still
...
not from SCOUT's own short record. The 1 °C threshold and the 4/8/12 bands are calibrated to satellite
