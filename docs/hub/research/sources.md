...
| Key | Work | Access | Local | Relevance / used in |
|---|---|---|---|---|
| `cfr-15-247` | 47 CFR §15.247 — "Operation within the bands 902–928 MHz, 2400–2483.5 MHz, and 5725–5850 MHz." [eCFR](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-A/part-15/subpart-C/subject-group-ECFR2f2e5828339709e/section-15.247) · [Cornell LII](https://www.law.cornell.edu/cfr/text/47/15.247) | 🔓 | — | **The governing rule.** (a)(1)(i) ≥50 hopping channels + ≤0.4 s occupancy per 20 s; (a)(2) ≥500 kHz minimum 6 dB bandwidth; (b)(2) power caps; (e) 8 dBm/3 kHz PSD; (f) hybrid systems — [notes](notes/cfr-15-247-ism-band-rules.md), [FCC 915 MHz Compliance](../../research/fcc-915-mhz-compliance.md) |
| `cfr-15-23` | 47 CFR §15.23 — "Home-built devices." [eCFR](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-A/part-15/subpart-A/section-15.23) | 🔓 | — | Exempts ≤5 non-marketed units from equipment **authorization**, not from the technical standards — why the capstone prototype needs no lab report but a sold product would — [notes](notes/cfr-15-23-home-built-exemption.md) |
...
| `cfr-15-249` | 47 CFR §15.249 — "Operation within the bands 902–928 MHz, 2400–2483.5 MHz, …" [eCFR](https://www.ecfr.gov/current/title-47/chapter-I/subchapter-A/part-15/subpart-C/section-15.249) | 🔓 | — | The **third** Part 15 route, evaluated and rejected: permits narrowband operation without §15.247's bandwidth/hopping constraints, but at field strengths roughly 20 dB below +14 dBm — [FCC 915 MHz Compliance §3](../../research/fcc-915-mhz-compliance.md) |
| `hi-dlnr-occl` | Hawaii DLNR Office of Conservation and Coastal Lands. [link](https://dlnr.hawaii.gov/occl/) | ❓ | — | Second permitting authority: governs **use of submerged land**, triggered by placing in-water structures. DAR refers applicants here — [SCO-101](https://linear.app/scout1/issue/SCO-101) |
| `fcc-2aseorfm95c` | FCC Grant of Equipment Authorization, **FCC ID 2ASEORFM95C** — RFM95C LoRa module, Shenzhen HOPE Microelectronics. Single Modular Approval, Part 15C, **equipment class DTS**. [grant](https://fccid.io/2ASEORFM95C) · [test report](https://fccid.io/2ASEORFM95C/Test-Report/Test-Report-4184769) · [RF exposure](https://cdn-shop.adafruit.com/product-files/3178/3178_exposure_report.pdf) | 🔓 | — | **The binding constraint on our radio.** Certified DTS (not FHSS) — so BW500 is the certified mode and hopping would void the grant. Caps conducted power at 0.0145 W (11.6 dBm) and antenna gain at 2.15 dBi; requires a "Contains FCC ID" label — [notes](notes/fcc-2aseorfm95c-modular-grant.md) |
| `fcc-kdb-996369` | FCC KDB 996369 D04 — Module Integration Guide. [pdf](https://apps.fcc.gov/kdb/GetAttachment.html?id=bNCiEdkFEKnHsZF9GHCNdg%3D%3D&desc=996369+D04+Module+Integration+Guide+V02&tracking_number=44637) | 🔓 | — | The integrator's obligations under a modular grant: output power and transmission parameters must stay within certified limits, or the approval is invalidated |
| `adafruit-3178` | Adafruit. "Feather M0 with RFM95 LoRa Radio — 900 MHz" (PID 3178), product page + [learn guide](https://learn.adafruit.com/adafruit-feather-m0-radio-with-lora-radio-module). [link](https://www.adafruit.com/product/3178) | 🔓 | — | The build platform ([ADR-0001](../../decisions/0001-mcu-and-radio-selection.md)). **Resolved 2026-09-01:** the shielded module carries FCC ID `2ASEORFM95C` — see [`fcc-2aseorfm95c`](#lora-phy--radio-engineering) for the conditions that bind |

## LoRa PHY & radio engineering

Primary vendor documentation for the SX1276/RFM95. These are the sources the modem
configuration and the airtime calculator are derived from — not secondary explainers.

| Key | Work | Access | Local | Relevance / used in |
|---|---|---|---|---|
| `semtech-an1200-13` | Semtech (2013). AN1200.13, "SX1272/3/6/7/8: LoRa Modem Designer's Guide." [pdf](https://www.mouser.com/pdfdocs/semtech-lora-modem-design.pdf) | 🔓 | — | **The airtime formula.** Symbol time, preamble, and payload-symbol equations behind [`scripts/lora_airtime.py`](../../../scripts/lora_airtime.py) and every airtime figure in the compliance analysis — [notes](notes/semtech-an1200-13-airtime.md) |
| `sx1276-datasheet` | Semtech. "SX1276/77/78/79 — 137 MHz to 1020 MHz Low Power Long Range Transceiver" datasheet. [pdf](https://cdn-shop.adafruit.com/product-files/3179/sx1276_77_78_79.pdf) | 🔓 | — | Register map for `RegModemConfig1/2/3` (0x1D/0x1E/0x26) — the derivation of `ModemConfig {0x98, 0xC4, 0x04}` in `firmware/src/config.h`, and the demodulator SNR limits per SF used in the link budget |
| `sx1276-errata` | Semtech. "Errata Note — SX1276/77/78," §2.1 *Sensitivity Optimization with 500 kHz Bandwidth*. Corroborated by [Adafruit_CircuitPython_RFM9x #51](https://github.com/adafruit/Adafruit_CircuitPython_RFM9x/issues/51) and the [Semtech SX1276Lib driver](https://os.mbed.com/teams/Semtech/code/SX1276Lib/file/7f3aab69cca9/sx1276/sx1276.cpp/) | ❓ | — | **Actionable gotcha:** at BW ≥ 500 kHz the receiver needs reg `0x36 = 0x02`, `0x3A = 0x64`. Receive-side only, so the TX-only buoy skips it and the shore station must not — [notes](notes/sx1276-errata-500khz.md), [SCO-24](https://linear.app/scout1/issue/SCO-24) |
| `semtech-an1200-26` | Semtech. AN1200.26, "LoRa and FCC Part 15.247: Measurement Guidance." [mirror](https://dtsheet.com/doc/1842140/lora%E2%84%A2-and-fcc-part-15.247--measurement-guidance) | ❓ | — | Semtech's own measurement guidance for demonstrating §15.247 compliance, including dwell-limited maximum payload. **Not yet read in full** — the canonical Semtech PDF was not retrievable; obtain before the compliance filing |
...
| `roark-8e` | Young, Budynas & Sadegh (2011). *Roark's Formulas for Stress and Strain*, 8th ed. McGraw-Hill. No DOI (handbook) | 🔒 | — | Standard closed-form basis for thin-shell membrane stress, external-pressure buckling of cylinders/curved panels, flat-plate bending, and shear-web buckling — [wedge wall-thickness check, 2026-09-07](../../../mechanical/test/wedge-wall-thickness-structural-check-2026-09-07.md) |
