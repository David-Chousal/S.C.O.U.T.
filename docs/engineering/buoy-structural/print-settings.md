...
|---|---|---|---|---|
| Wedge (shell) — **v5, 2026-09-07** | perimeters only | 0% (solid perimeters) | **outer curved wall 0.095 in / 2.41 mm (~6 passes); radial side walls + internal web 0.063 in / 1.60 mm (~4 passes) — recommend the sides go to 0.080 in / 2.00 mm** | Non-primary, **foam-backed and now foam-dependent for the outer wall against external pressure** — see [wedge wall-thickness check, 2026-09-07](../../../mechanical/test/wedge-wall-thickness-structural-check-2026-09-07.md). Internal bracing web is **mandatory** (DFM: thin open walls flexed on the bed; also halves every panel span). Foam must be in before any submersion. Bolt loads on the ~0.69 in inner flange only |
...
the outer wall passes with margin foam-backed; the side walls are marginal at 0.063 in and
should go to 0.080 in. **Not yet FEA-verified, and not frozen** until the v5 STEP is exported,
the [SCO-110](https://linear.app/scout1/issue/SCO-110) mass recompute is done, and
