...
>
> **Corrected 2026-09-13:** the SEN0189 turbidity adapter board was listed below as occupying
> volume in this housing. Confirmed with John that it actually lives in the **sensor pod's dry
> chamber** (see [Sensor Pod Packing Budget](sensor-pod-packing-budget.md)), not here — removed
> from the manifest and Methods 1/3 recomputed. This document's own contradiction with
> [`mechanical/cad/sensor-housing/README.md`](../../mechanical/cad/sensor-housing/README.md)
> (which always said the pod holds this board) went unflagged since 2026-08-25; caught while
> building the sensor pod's own packing analysis.
...
| SEN0189 turbidity adapter board + probe | **Not housed here** — lives in the sensor pod's dry chamber, per [Sensor Pod Packing Budget](sensor-pod-packing-budget.md). Only the pod's own signal wire enters this housing, absorbed into the wiring margin below | — | See §1 correction note above |
...
**One constraint worth flagging before the geometry:**
...
| Length (3 slices, SEN0189 removed 2026-09-13) | 22 + 10 + 5 (thicknesses) + 2×6 (gaps) + 2×15 (end allowances) = **79 mm** | [A] |
| **Volume** | π × 32.5² × 79 = **262,000 mm³ ≈ 0.262 L** | [X] (Archimedes-style, exact given the inputs) |

Smallest volume and slimmest diameter of the three methods, at the cost of the longest tube.

## 4. Method 2 — Single flat layer

Three boards side by side on one tray, occupying a single axial position (2026-09-13: originally
a 2×2 grid with SEN0189 in a second row — recomputed as a single row now that SEN0189 has moved
to the sensor pod):

```
Row width  = 23 + 25 + 30 + 2×6 (gaps) = 90 mm,  row height = 51 mm (Feather+Wing's long edge)
Diagonal = sqrt(90² + 51²) = sqrt(8100 + 2601) = sqrt(10701) = 103.4 mm → D = 111.4 mm
```

| Result | Value | Tag |
|---|---|---|
| **Governing diameter** | **112 mm** | [A] |
| Length | 22 (tallest board) + 3 (tray) + 5 (clearance) + 2×15 (end allowances) = **60 mm** | [A] |
| **Volume** | π × 56² × 60 = **591,400 mm³ ≈ 0.591 L** | [X] |

Shortest tube of the three, but **still needs a bigger diameter than the ~4″ Schedule 40 PVC
reference already assumed in [`mechanical/README.md`](../../mechanical/README.md)** — and pays
heavily for it, since volume scales with the *square* of radius but only linearly with length.
A tighter, non-grid nesting of the three boards could shrink this diameter, but that needs real
CAD placement, not hand geometry — flagged as an open item in [§7](#7-open-items).

## 5. Method 3 — Two-zone hybrid

Feather+Wing paired with PID6106 in zone 1; the battery alone in zone 2 (2026-09-13: zone 2
originally paired the battery with SEN0189, now moved to the sensor pod). Zone 1's pair sits
side by side; the two zones stack along the tube's axis.
...
Zone 2: width = 35 mm, height = 30 mm (battery alone)
        diagonal = sqrt(35² + 30²) = sqrt(1225 + 900) = sqrt(2125) = 46.1 mm → D = 54.1 mm
        zone thickness = 5 mm (battery thickness)
```

| Result | Value | Tag |
|---|---|---|
| **Governing diameter** | **83 mm** (zone 1 — zone 2 shrank well below it once SEN0189 left) | [A] |
| Length | 22 + 6 (gap) + 5 + 2×15 (end allowances) = **63 mm** | [A] |
| **Volume** | π × 41.5² × 63 = **340,700 mm³ ≈ 0.341 L** | [X] |
...
[§7](#7-open-items)). ⚠️ **The SVG below still shows the pre-2026-09-13 layout (SEN0189 in
zone 2) and needs re-export** — the character version underneath it is corrected and is the
one to read until then:
...
```
ELECTRONICS HOUSING — TWO-ZONE HYBRID PACKING (Method 3, corrected 2026-09-13)
  ...
  (⌀83mm ID)                              (⌀83mm ID)
    ...
    /                     \                  /                     \
   /   +-------+ +-------+ \                /      +--------+       \
  |    |M0+Wing| |PID6106| |                |       |Battery |       |
  |    |51x23mm| |~51x25 | |                |       |35x30mm |       |
  |    |  mm   | |  mm   | |                |       |        |       |
  |    +-------+ +-------+ |                |       +--------+       |
   \                      /                  \                     /
    \                    /                    \                   /
...
AXIAL VIEW (side profile, cylinder length)
   ...
   |  cap   | M0+Wing +   |  --  | Battery  |  cap   |
   | (gland)|  PID 6106   |      |  alone   |(gland) |
   | 15 mm  |   22 mm     | 6 mm |   5 mm   | 15 mm  |
   +--------+-------------+------+----------+--------+
   |<---------------- 63 mm bare minimum ------------------>|
...
|---|---|---|---|---|
| 1 — Axial stack | 65 mm | 79 mm | 0.262 L | Yes, 37 mm to spare |
| 2 — Single flat layer | 112 mm | 60 mm | 0.591 L | **No** — 10 mm over |
| 3 — Two-zone hybrid | 83 mm | 63 mm | 0.341 L | Yes, 19 mm to spare |

## 6. Recommendation

**Method 3.** It's about 30% more volume than Method 1 but ~20% shorter, and — the useful part —
...
volume-efficient on paper but produces a notably long, thin tube for a modest length cost saved
against Method 3.

**This is a floor, not a target.** The bare-minimum computed length (63 mm) leaves no slack for
...
existing ~100 mm diameter** is what this recommends actually designing to, not the bare 63 mm
minimum.
...
- **SVG diagram needs re-export** — `assets/diagrams/electronics-housing-packing-diagram.svg`
  still shows the pre-2026-09-13 layout with SEN0189 in zone 2. The character-art version in
  [§5](#5-method-3--two-zone-hybrid) is corrected; the SVG isn't yet.
